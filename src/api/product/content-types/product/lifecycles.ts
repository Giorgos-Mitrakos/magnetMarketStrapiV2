export default {
  ...
  async beforeDeleteMany(event) {
    for (let id of event.params.where.$and[0].id.$in) {
      ...
      try {
        ...
        if (entry.additionalFiles) {
          const fileEntry = await strapi.db
            .query("plugin::upload.file")
            .delete({
              where: { id: entry.additionalFiles.id },
            });
  ...
  async beforeUpdate(event) {
    ...
    if (entry.is_archived && entry.deletedAt === null) {
      ...
      strapi.log.info(
        `[Lifecycle] Product ${entry.id} back in stock - cleared is_archived`
      );
    }
  ...
  async afterUpdate(event) {
    ...
    try {
      // 1. Φέρνουμε το service
      const cacheService = strapi
        .plugin("import-products")
        .service("cacheService");

      // 2. ΕΛΕΓΧΟΣ: Είναι το προϊόν σε φάση import;
      const isImporting = cacheService.cache.processingProducts.has(result.id);

      if (!isImporting) {
        // ✅ ΑΥΤΟ ΤΡΕΧΕΙ ΜΟΝΟ ΣΕ ΧΕΙΡΟΚΙΝΗΤΕΣ ΑΛΛΑΓΕΣ
        // Εφόσον δεν είναι στο Set του import, σημαίνει ότι η αλλαγή έγινε από το Admin
        try {
          console.log(
            `[Manual Update] Invalidating cache for product ${result.id}`
          );
          await cacheService.invalidateProduct(result.id);
        } catch (error) {
          strapi.log.debug(
            `Manual cache invalidation failed: ${error.message}`
          );
        }
      }
    } catch (error) {
      ...
      strapi.log.debug(
        `Cache invalidation skipped for product ${result.id}: ${error.message}`
      );
    }
    ...
    const productBecameAvailable =
      oldData && productService.checkProductAvailabilityChange(oldData, result);
    ...
    if (supplierInfoChanged) {
      ...
      if (isInStock) {
        ...
        try {
          setImmediate(async () => {
            try {
              const analyzer = strapi
                .plugin("bargain-detector")
                ?.service("analyzer");

              if (!analyzer) {
                strapi.log.error(
                  "[Lifecycle] Bargain detector analyzer service not found"
                );
                return;
              }

              strapi.log.debug(
                `[Lifecycle] Queuing auto-analysis for product ${result.id}`
              );
              ...
              strapi.log.info(
                `[Lifecycle] ✓ Auto-analyzed product ${result.id} after supplier info update`
              );
            } catch (error) {
              strapi.log.error(
          ...
          strapi.log.error(
            `[Lifecycle] Failed to queue analysis: ${error.message}`
          );
        }
      } else {
        strapi.log.debug(
    ...
    if (productBecameAvailable) {
      strapi.log.info(
        `[Lifecycle] Product ${result.id} became available for customers, triggering notify_me...`
      );
      ...
      setTimeout(async () => {
        try {
          ...
          if (typeof notifyService.notifyProductAvailable !== "function") {
            strapi.log.error(
              "[Lifecycle] notifyProductAvailable method not found"
            );
            return;
          }

          const notificationResult = await notifyService.notifyProductAvailable(
            result.id
          );
          ...
          if (notificationResult?.success !== false) {
            strapi.log.info(
              `[Lifecycle] ✓ Notified ${
                notificationResult?.notified || 0
              } customers for product ${result.id}`
            );
          }
        } catch (error) {
          strapi.log.error(
            `[Lifecycle] Failed to notify customers for product ${result.id}:`,
            error
          );
        }
      }, 3000);
