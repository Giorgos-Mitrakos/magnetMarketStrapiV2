...
module.exports = ({ strapi }) => ({
  async updateAll() {
    await updateZEGETRON()
      .then(async () => {
        return await updateOKTABIT();
      })
      .then(async () => {
        return await updateWESTNET();
      })
      .then(async () => {
        return await scrapQUEST();
      });

    async function updateZEGETRON() {
      const entry = await strapi.db
        .query("plugin::import-products.importxml")
        .findOne({
          where: { name: "Zegetron" },
          populate: {
            importedFile: true,
  ...
  async createImportRef(entry) {
    ...
    importRef.charMaps = strapi
      ...
      .parseCharsToMap(
        importRef.categoryMap.char_name_map,
        importRef.categoryMap.char_value_map
      );

    importRef.suppliers = await strapi.entityService.findMany(
    ...
    importRef.stock_map = await strapi.db
      .query("plugin::import-products.stockmap")
      .findMany({
    ...
    importRef.brand_excl_map = await strapi.db
      .query("plugin::import-products.brandexclmap")
      .findMany({
        where: { related_import: entry.id },
  ...
  async getXmlData(url, config) {
    ...
    try {
      ...
      logger.logToFile(`Error in getData: ${error}`);
  ...
  async createEntry(product, importRef) {
    if (!product.name || (!product.mpn && !product.barcode)) return;
    ...
    if (this.isBrandBlocked(newProductBrandName, importRef.brand_excl_map)) {
      console.log(
        `🚫 Brand blocked, skipping create: ${product.name} (${newProductBrandName})`
      );
      importRef.skipped += 1;
    ...
    try {
      const categoryInfo = await strapi
        ...
        .getCategory(
          importRef.categoryMap.categories_map,
          product.name,
          product.category.title,
          product.subcategory.title,
          product.sub2category.title
        );
      ...
      let platforms = [productPrices.skroutzPrice, productPrices.shopflixPrice];
      ...
      data.deletedAt =
        product.status === "OutOfStock" || product.status === "Discontinued"
          ? new Date()
          : null;
    ...
    try {
      ...
      if (this.isBrandBlocked(updateBrandName, importRef.brand_excl_map)) {
        if (entryCheck.publishedAt !== null) {
          ...
          console.log(
            `🚫 Brand blocked, unpublished: ${entryCheck.name} (${updateBrandName})`
          );
          importRef.updated += 1;
        } else {
          importRef.skipped += 1;
        ...
        return {
          success: true,
          changeType: entryCheck.publishedAt !== null ? "updated" : "Skipped",
        };
      }
      ...
      await this.updateProductMetadata(
        entryCheck,
        product,
        categoryInfo,
        data,
      ...
      const skroutz = entryCheck.platforms.find(
        (x) => x.platform === "Skroutz"
      );
      const shopflix = entryCheck.platforms.find(
        (x) => x.platform === "Shopflix"
      );

      let info = data.supplierInfo
        ? data.supplierInfo
        : entryCheck.supplierInfo;
      ...
      if (
        ...
        shopflix.is_fixed_price !==
          productPrices.shopflixPrice.is_fixed_price ||
        strapi
          .plugin("import-products")
          .service("priceHelpers")
      ...
      const productForStatus = {
        ...product,
        brandName:
          product.brandName || entryCheck.brand?.name || entryCheck.brand,
      };

      const supplierInfoForStatus =
        data.supplierInfo || entryCheck.supplierInfo;
      const inventoryForStatus =
        data.inventory !== undefined ? data.inventory : entryCheck.inventory;
      ...
      const finalStatus =
        data.status !== undefined ? data.status : entryCheck.status;
  ...
  async deleteEntry(entry, importRef) {
    try {
      const importXmlFile = await strapi.entityService.findOne(
        "plugin::import-products.importxml",
        entry.id,
        {
          populate: {
      ...
      for (let product of importXmlFile.related_products) {
        if (!importRef.related_entries.has(product.id)) {
          ...
          const checkProduct = await strapi.entityService.findOne(
            "api::product.product",
            product.id,
            {
              populate: {
          ...
          if (index === -1) {
            let relatedImports = checkProduct.related_import.filter(
              (x) => x.id !== entry.id
            );
          ...
          const finalStatus =
            data.status !== undefined ? data.status : checkProduct.status;
  ...
  updateImportReferences(importRef, entryCheck, product, data) {
    ...
    const findImport = relatedImport.findIndex(
      (x) => x.id === product.entry.id
    );
  ...
  async handleAvailabilityNotifications(entryCheck, product, data) {
    ...
    const allSuppliersWereUnavailable = oldSupplierInfo.every(
      (s) => !AVAILABLE_STATUSES.includes(s.translated_status)
    );
    ...
    const newlyAvailableSuppliers = newSupplierInfo.filter((newSupp) => {
      ...
      return (
        !oldSupp || !AVAILABLE_STATUSES.includes(oldSupp.translated_status)
      );
    });
    ...
    const emailVariables = {
      product: {
        ...
        supplierProductId:
          supplierThatCameBack.supplierProductId ||
          ...
          "-",
        supplierPrice:
          supplierThatCameBack.wholesale || supplierThatCameBack.price || "-",
        supplierRetailPrice: supplierThatCameBack.retail_price || "-",
        totalSuppliersReturned: newlyAvailableSuppliers.length,
        allSuppliers: newlyAvailableSuppliers
          .map((s) => `${s.name} (${s.translated_status})`)
          .join(", "),
      },
    };
    ...
    try {
      await strapi.service("api::order.order").sendConfirmOrderEmail({
        ...
        to: [
          "giorgos_mitrakos@yahoo.com",
          "info@magnetmarket.gr",
          "kkoulogiannis@gmail.com",
        ],
        emailVariables,
      ...
      strapi.log.error(
        `[Import] ❌ Failed to send notification for product ${entryCheck.id}:`,
        error
      );
    }
  },
  ...
  async updateProductMetadata(
    ...
    try {
      ...
      ["length", "width", "height"].forEach((dim) => {
        ...
        if (
          (!entryCheck[dim] && ceiledValue > 0) ||
          (entryCheck[dim] &&
            entryCheck[dim] !== ceiledValue &&
            ceiledValue > 0)
        ) {
  ...
  createSlug(name, mpn) {
    try {
      ...
      const slug = slugify(`${newName}-${mpn}`, {
        lower: true,
        remove: /[^A-Za-z0-9_.~\s]/g,
      });
      return slug;
  ...
  async deleteNonRelatedProducts() {
    ...
    const nonRelatedProducts = await strapi.entityService.findMany(
      "api::product.product",
      {
    ...
    for (let product of nonRelatedProducts) {
      ...
      if (product.supplierInfo) {
        ...
        if (newSuppliers.length > 0) {
          ...
          const isAllSuppliersOutOfStock = data.supplierInfo.every(
            (supplier) => supplier.in_stock === false
          );
  ...
  isBrandBlocked(brandName, brand_excl_map) {
    ...
    const entry = brand_excl_map.find(
      (b) =>
  ...
  async unpublishBlockedBrandProducts(entry) {
    try {
      const blockedBrands = await strapi.db
        .query("plugin::import-products.brandexclmap")
        .findMany({
          where: {
      ...
      if (!blockedBrands?.length) {
        console.log(
          `[BrandBlock] No blocked brands for supplier: ${entry.name}`
        );
        return { unpublished: 0 };
      }

      const blockedBrandNames = blockedBrands.map((b) =>
        b.brand_name.toLowerCase().trim()
      );
      console.log(
        `[BrandBlock] Checking brands: ${blockedBrandNames.join(", ")}`
      );
      ...
      while (true) {
        ...
        for (const product of products) {
          ...
          try {
            ...
            unpublishedCount++;
            console.log(
              `🚫 [BrandBlock] Unpublished: ${product.name} (${product.brand?.name})`
            );
          } catch (error) {
            console.error(
              `[BrandBlock] Failed to unpublish product ${product.id}:`,
              error.message
            );
          }
        }
      ...
      console.log(
        `[BrandBlock] Done. Unpublished ${unpublishedCount} products for supplier: ${entry.name}`
      );
      return { unpublished: unpublishedCount };
    } catch (error) {
      console.error(
        "[BrandBlock] unpublishBlockedBrandProducts failed:",
        error.message
      );
      return { unpublished: 0 };
    }
  ...
  async republishUnblockedBrandProducts(entry) {
    try {
      const allowedBrands = await strapi.db
        .query("plugin::import-products.brandexclmap")
        .findMany({
          where: {
            related_import: entry.id,
      ...
      if (!allowedBrands?.length) {
        console.log(
          `[BrandUnblock] No allowed brands for supplier: ${entry.name}`
        );
        return { republished: 0 };
      }

      const allowedBrandNames = allowedBrands.map((b) =>
        b.brand_name.toLowerCase().trim()
      );
      console.log(
        `[BrandUnblock] Checking brands: ${allowedBrandNames.join(", ")}`
      );
      ...
      while (true) {
        ...
        for (const product of products) {
          ...
          try {
            ...
            const hasAvailableSupplier = product.supplierInfo?.some(
              (s) => s.in_stock === true
            );
            ...
            if (hasAvailableSupplier) {
              republishedCount++;
              console.log(
                `✅ [BrandUnblock] Republished: ${product.name} (${product.brand?.name}) → ${calculatedStatus}`
              );
            } else {
              console.log(
                `⏭️ [BrandUnblock] Skipped (no available supplier): ${product.name}`
              );
            }
          } catch (error) {
            console.error(
              `[BrandUnblock] Failed to republish product ${product.id}:`,
              error.message
            );
          }
        }
      ...
      console.log(
        `[BrandUnblock] Done. Republished ${republishedCount} products for supplier: ${entry.name}`
      );
      return { republished: republishedCount };
    } catch (error) {
      console.error(
        "[BrandUnblock] republishUnblockedBrandProducts failed:",
        error.message
      );
      return { republished: 0 };
    }
