...
module.exports = ({ strapi }) => {
  const { BaseSupplier } = strapi
    .plugin("import-products")
    .service("baseSupplier");
  ...
  class IasonAdapter extends BaseSupplier {
    ...
    async transformProduct(product, rawData, importRef) {
      ...
      product.recycle_tax =
        parseFloat(this.cleanPrice(product.recycle_tax)) || 0; // ✅ number όχι string

      product.description = "";

      if (product.weight) {
        const weightKg = parseFloat(
          String(product.weight).replace(",", ".").trim()
        );
