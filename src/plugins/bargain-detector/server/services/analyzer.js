...
module.exports = ({ strapi }) => ({
  ...
  async analyzeAndStore(productId, options = {}) {
    ...
    try {
      ...
      const product = await strapi.entityService.findOne(
        ...
        productId,
        {
          populate: {
            ...
            // 🔒 LIQUIDITY_TRACKING_ENABLED = false στο metrics.js

            // Uncomment όταν ενεργοποιηθεί το liquidity tracking:
      ...
      strapi.log.debug(
        `[Analyzer] Analyzing product ${productId}: ${product.name}`
      );
      ...
      const analysis = await opportunityAnalyzer.analyzeProduct(
        product,
        options
      );
      ...
      strapi.log.info(
        `[Analyzer] ✓ Product ${productId} analyzed and stored in ${
          Date.now() - startTime
        }ms`
      );
      ...
      strapi.log.error(
        `[Analyzer] Failed to analyze product ${productId}: ${error.message}`
      );
      throw error;
    }
  },
  ...
  async storeOpportunity(product, analysis, options = {}) {
    ...
    if (existing && existing.length > 0) {
      ...
      const scoreChange = Math.abs(
        existingOpp.opportunity_score - data.opportunity_score
      );
  ...
  async analyzeBatchAndStore(productIds, options = {}) {
    ...
    strapi.log.info(
      `[Analyzer] Starting batch run ${analysisRun.id} for ${productIds.length} products`
    );
    ...
    try {
      ...
      const avgOppScore =
        results.success.length > 0
          ? results.success.reduce((sum, r) => sum + r.opportunity_score, 0) /
            results.success.length
          : 0;

      const avgRiskScore =
        results.success.length > 0
          ? results.success.reduce((sum, r) => sum + r.risk_score, 0) /
            results.success.length
          : 0;
      ...
      strapi.log.info(
        `[Analyzer] Batch run ${analysisRun.id} completed: ` +
          `${results.success.length}/${productIds.length} in ${
            Date.now() - startTime
          }ms`
      );
   ...
   */
  /**
   * Καθορίζει αν πρέπει να σταλεί ειδοποίηση (email/push) για μια ευκαιρία.
   * ✅ Βελτιστοποιημένο για άμεση ειδοποίηση σε περίπτωση Clearance.
   */
  shouldNotify(opportunity, config) {
    // 1. Έλεγχος αν έχει ήδη σταλεί ειδοποίηση για να αποφύγουμε το spam[cite: 10]
    ...
    const clearanceData = analysisData.clearance_detection;

    // ✅ 2. ΕΙΔΟΠΟΙΗΣΕΙΣ CLEARANCE (Υψηλή Προτεραιότητα)[cite: 13]
    if (clearanceData && alerts.clearance_sales?.enabled) {
      // ΠΑΡΑΚΑΜΨΗ: Αν ο Analyzer έβγαλε recommendation "clearance_urgent",
      // στέλνουμε ειδοποίηση ΠΑΝΤΑ, ακόμα και με χαμηλό confidence (π.χ. 40%)[cite: 13]
      if (opportunity.recommendation === "clearance_urgent") {
        strapi.log.info(
          `[Notifier] Force notifying for urgent clearance: ${opportunity.id}`
        );
        return true;
      }

      const minConfidence = alerts.clearance_sales.min_confidence || 50;
      const sendAll = alerts.clearance_sales.send_all !== false;

      // Έλεγχος βάσει ρυθμίσεων confidence[cite: 10]
      if (sendAll || clearanceData.confidence >= minConfidence) {
        return true;
      }
    }

    // 3. FLASH DEALS (Απότομες πτώσεις τιμής)[cite: 13]
    if (alerts.flash_deals?.enabled) {
      const hasFlashDeal = analysisData.key_insights?.some(
        (insight) => insight.type === "flash_opportunity"
      );

      if (
        hasFlashDeal &&
        opportunity.opportunity_score >= (alerts.flash_deals.min_score || 85)
      ) {
        return true;
      }
    }

    // 4. ΚΡΙΣΙΜΕΣ ΕΥΚΑΙΡΙΕΣ (Βάσει Priority και Score)[cite: 13]
    if (alerts.critical_opportunities?.enabled) {
      if (
        opportunity.priority === "critical" ||
        (opportunity.opportunity_score >=
          (alerts.critical_opportunities.min_score || 90) &&
    ...
    // Αν δεν πληρούται καμία από τις παραπάνω προϋποθέσεις[cite: 10]
    return false;
  },
  ...
  async sendNotification(opportunity, product, config) {
    try {
      ...
      // Critical/Flash
        ...
        opportunity.priority === "critical" ||
        opportunity.priority === "flash_clearance"
      ) {
        channels.push(
          ...(alerts.critical_opportunities?.channels || ["email"])
        );
      }
      ...
      strapi.log.info(
        `[Analyzer] Notifications sent via: ${uniqueChannels.join(", ")}`
      );
    } catch (error) {
  ...
  async sendClearanceEmail(opportunity, product, clearanceData) {
    ...
    try {
      const recDetails =
        opportunity.analysis_data?.recommendation_details || {};
      const actions = opportunity.analysis_data?.action_items || [];

      // Build signals list
      const signalsList = clearanceData.signals
        .map(
          (s) =>
            `<li><strong>${s.type.replace(/_/g, " ").toUpperCase()}:</strong> ${
              s.message
            }</li>`
        )
        .join("");
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #fff; padding: 30px; border: 2px solid #ff6b6b;">
            <div style="background: #fff3cd; padding: 15px; border-left: 4px solid #ff6b6b; margin-bottom: 20px;">
              ...
              <p style="margin: 5px 0 0 0; color: #666;">
                Confidence: ${
                  clearanceData.confidence
                }% | Urgency: ${clearanceData.urgency.toUpperCase()}
              </p>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                ...
                <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: right; color: #28a745; font-weight: bold;">${
                  opportunity.opportunity_score
                }/100</td>
              </tr>
              <tr>
                <td style="padding: 10px; border-bottom: 1px solid #ddd;"><strong>Risk Score:</strong></td>
                <td style="padding: 10px; border-bottom: 1px solid #ddd; text-align: right; color: #dc3545; font-weight: bold;">${
            ...
            ${
              ...
              <ol style="line-height: 1.8; color: #555;">
                ${actions
                  .map(
                    (a) =>
                      `<li><strong>${a.description || a.action}</strong>${
                        a.note
                          ? `<br><small style="color: #888;">${a.note}</small>`
                          : ""
                      }</li>`
                  )
                  .join("")}
            ...
            <div style="background: #ffebee; padding: 15px; border-radius: 8px; margin-top: 30px; border-left: 4px solid #d32f2f;">
              ...
              <p style="margin: 5px 0 0 0; color: #666;">
                Typical clearance window: ${
                  clearanceData.recommendation?.estimated_window || "5-10 days"
            ...
            <div style="text-align: center; margin-top: 30px;">
              <a href="${
                process.env.STRAPI_ADMIN_URL || "https://magnetmarket.gr"
              }/admin/plugins/bargain-detector/opportunities/${opportunity.id}" 
          ...
          <div style="background: #f5f5f5; padding: 20px; text-align: center; border-radius: 0 0 10px 10px; margin-top: 2px;">
            <p style="margin: 0; color: #888; font-size: 12px;">
              ...
              <a href="${
                process.env.STRAPI_ADMIN_URL || "https://magnetmarket.gr"
              }/admin/plugins/bargain-detector/opportunities/${
        opportunity.id
      }" style="color: #ff6b6b;">
      ...
      strapi.log.info(
        `[Analyzer] Clearance email sent for opportunity ${opportunity.id}`
      );
    } catch (error) {
  ...
  async sendEmail(opportunity, product) {
    ...
    try {
      ...
      const html = `
        ...
        ${
          recDetails.suggested_stock_days
            ? `<p><strong>Suggested Stock:</strong> ${recDetails.suggested_stock_days} days</p>`
            : ""
        }
        ...
        <h3>Key Insights</h3>
        <ul>${insights
          .map((i) => `<li><strong>${i.type}:</strong> ${i.message}</li>`)
          .join("")}</ul>
        
        <h3>Recommended Actions</h3>
        <ul>${actions
          .map((a) => `<li>${a.description || a.action}</li>`)
          .join("")}</ul>
        
        <p><a href="${
          process.env.STRAPI_ADMIN_URL || "https://magnetmarket.gr"
        }/admin">View in Dashboard</a></p>
      `;
      ...
      await strapi
        ...
        .send({
          ...
          subject: `${
            opportunity.priority === "critical" ? "🚨 URGENT" : "💰"
          } Bargain Alert - ${product.name}`,
          html,
        });

      strapi.log.info(
        `[Analyzer] Email sent for opportunity ${opportunity.id}`
      );
    } catch (error) {
  ...
  async cleanup() {
    try {
      ...
      const deletedOpps = await strapi.db
        .query("plugin::bargain-detector.bargainopportunity")
        .deleteMany({
          where: {
      ...
      const deletedRuns = await strapi.db
        .query("plugin::bargain-detector.analysisrun")
        .deleteMany({
          where: {
