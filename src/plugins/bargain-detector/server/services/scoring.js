...
module.exports = ({ strapi }) => ({
   ...
   * ✅ NEW: Accepts clearanceDetection parameter
   */
  async calculateScores(product, metrics, patterns = [], clearanceDetection = null) {
    const helpers = strapi.plugin('bargain-detector').service('helpers');
    const config = await helpers.loadConfig();

    // Calculate Opportunity Score (✅ with clearance boost)
    ...
    // Calculate Confidence (✅ with clearance boost)
    ...
    // Calculate Priority (✅ clearance can override)
  ...
  calculateOpportunityScore(metrics, patterns, config, clearanceDetection) {
    ...
    const timing = this.calculateTiming(metrics, patterns);
    ...
    return {
      ...
      breakdown: {
        ...
        clearance_boost: clearanceDetection ? {
          ...
          details: `+${clearanceBoost.toFixed(1)} points from clearance detection`
        } : null
      }
    };
  },
   ...
   * ✅ ENHANCED: Uses per-supplier trend data
   */
  calculateTiming(metrics, patterns) {
    ...
    // ✅ NEW: Check best supplier trend + market trend
    ...
    const bestSupplierTrend = bestSupplierAnalysis?.trend?.direction || trend.direction;

    // Best supplier trend (primary signal)
    ...
    // ✅ NEW: Market-wide trend bonus (if multiple suppliers dropping)
    const droppingSuppliers = supplierAnalysis.filter(s => s.hasData && s.isDropping).length;
  ...
  calculateRiskScore(product, metrics, config) {
    ...
    const supplierReliability = this.calculateSupplierReliabilityRisk(product, metrics, config);
   ...
   * ✅ ENHANCED: Uses best supplier volatility primarily
   */
  calculateVolatilityRisk(metrics, config) {
    ...
    // ✅ NEW: Check best supplier volatility first
    const supplierAnalysis = metrics.supplierAnalysis || [];
    const bestSupplierAnalysis = supplierAnalysis.find(s =>
      s.supplier.name === metrics.bestSupplier && s.hasData
    );

    const cv = bestSupplierAnalysis?.coefficientOfVariation || metrics.coefficientOfVariation || 0;
    ...
    // ✅ NEW: Market volatility check (all suppliers)
   ...
   * ✅ COMPLETELY REWRITTEN: Uses per-supplier analysis
   */
  calculateSupplierReliabilityRisk(product, metrics, config) {
    ...
    if (!bestSupplierAnalysis) {
      return {
        ...
        percentage: "100.0",
        details: [{
          factor: 'no_supplier_data',
          points: 30,
          interpretation: 'Cannot assess - no supplier analysis available'
        }]
      };
    }
    ...
    // B. Error Rate (0-12)
    // ✅ Uses per-supplier anomaly count
    ...
    // C. Consistency (0-8)
    // ✅ Uses per-supplier consistency score
   ...
   * ✅ ENHANCED: Clearance boosts confidence
   */
  calculateConfidence(product, metrics, patterns, config, clearanceDetection) {
    ...
    const supplierAnalysis = metrics.supplierAnalysis || [];
    const reliableSuppliers = supplierAnalysis.filter(s => {
      if (!s.hasData || s.dataPoints < 10) return false;
      const dataQuality = s.dataQuality || 0;
      return dataQuality >= 0.6;
    });

    const totalSuppliersWithData = supplierAnalysis.filter(s => s.hasData).length;

    if (totalSuppliersWithData > 0) {
      const reliabilityRatio = reliableSuppliers.length / totalSuppliersWithData;
    ...
    // ✅ NEW: D. Clearance Detection Boost (0-2)
    if (clearanceDetection) {
      const clearanceConfidence = clearanceDetection.confidence / 100;
      const clearancePoints = clearanceConfidence * 2;
      ...
      details.push({
        ...
        interpretation: 'Strong signal from multiple clearance indicators'
      });
    }

    // Convert to enum (max 12 now instead of 10)
    let confidenceEnum;
    if (score >= 10) confidenceEnum = 'very_high';
    else if (score >= 8) confidenceEnum = 'high';
    else if (score >= 6) confidenceEnum = 'medium';
  ...
  determineRecommendation(opportunityScore, riskScore, confidence, metrics, config) {
    ...
    if (
      ...
      return {
        ...
        recommendation_note: isFastMover
    ...
    if (
      ...
      return {
        ...
        recommendation_note: 'Cautious approach - verify order before purchasing'
      };
    }
  ...
  /**
   * Calculate Priority
   * ✅ ENHANCED: Clearance can set flash_clearance priority
   */
  calculatePriority(opportunityScore, riskScore, metrics, clearanceDetection) {
    // ✅ HIGHEST PRIORITY: Flash clearance
    if (clearanceDetection && clearanceDetection.urgency === 'critical' && clearanceDetection.confidence >= 70) {
